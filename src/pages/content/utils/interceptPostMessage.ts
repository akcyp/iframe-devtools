...
export const interceptIframePostMessage = (contentWindow: Window) => {
  ...
  contentWindow.postMessage = function () {
    ...
    const actualTargetOrigin =
      typeof targetOrigin === 'string' ? targetOrigin : targetOrigin?.targetOrigin || '*';

    window.dispatchEvent(
      new CustomEvent('__IFRAME_INSPECTOR_MESSAGE__', {
        detail: {
          type: 'postMessage',
          timestamp: Date.now(),
          direction: 'sent',
